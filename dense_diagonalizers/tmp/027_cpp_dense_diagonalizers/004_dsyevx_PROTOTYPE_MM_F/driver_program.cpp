...
int32_t diagonalize_matrix(std::vector<double> &matrix,
    ...
    info = LAPACKE_dsyevx(LAPACK_ROW_MAJOR, 'V', 'I', 'U',
                          dimen, &matrix[0], lda,
                          vl, vu, il, iu, abstol, &m,
                          &eigenvalues[0], &eigenvectors[0], ldz, &failed[0]);
...
int main() {
    ...
    // container for the failed cases

    std::vector<int32_t> failed;
    failed.resize(static_cast<uint64_t>(DIMEN));
    ...
    auto info = diagonalize_matrix(matrix, eigenvalues, eigenvectors, num_eigenvalues_found, failed);
